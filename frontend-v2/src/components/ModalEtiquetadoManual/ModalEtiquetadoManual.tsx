...
const ModalEtiquetadoManual: React.FC = () => {
  ...
  const setProductosByFamilia = (familia: string) => {
    const newProductos = productosLegacy.filter(
      (p: any) => p.familia_producto === familia
    );
    setProductos(
      newProductos.sort((a: any, b: any) =>
  ...
  const setEansByProductos = (id_producto: string) => {
    const newEans = eansLegacy.filter(
      (e: any) => e.id_producto === id_producto
    );
    setEans(
      newEans.sort((a: any, b: any) => a.codigo_ean.localeCompare(b.codigo_ean))
    );
    return newEans;
  };

  const setPresentacionesByEans = (eans: any[]) => {
    const presentacionesIDs = eans.map((e: any) => e.id_presentacion);
    const newPresentaciones = presentacionesLegacy.filter((p: any) =>
      presentacionesIDs.includes(p.id_presentacion)
    );
    setPresentaciones(
      newPresentaciones.sort((a: any, b: any) =>
        a.nombre_presentacion.localeCompare(b.nombre_presentacion)
      )
    );
  };

  const handleChange = (campo: keyof FormData, valor: any) => {
    setFormData((prev) => ({
      ...prev,
      [campo]: valor,
    }));
  ...
  const validateForm = (formData: FormData): ErrorMap | null => {
    ...
    if (!formData.numeroBidon || isNaN(Number(formData.numeroBidon))) {
      ...
      const valor = parseInt(formData.numeroBidon as any, 10);
      if (valor < 1) {
        errors.numeroBidon = i18n.modalEtiquetadoManual.numeroBidonInvalid;
    ...
    if (
      ...
      const valor = parseInt(formData.intervaloMinutos as any, 10);
    ...
    if (!formData.code) {
      ...
      const regexCode = /^\d{3}$/;
      if (!regexCode.test(formData.code.toString())) {
  ...
  useEffect(() => {
    ...
    const presentacionSeleccionada = presentaciones.find(
      (p: any) => p.id_presentacion === formData.idPresentacion
    );
  ...
  const calculateCode = (fechaProduccion: string) => {
  ...
  useEffect(() => {
    ...
    const esEntero = (v: any) => Number.isInteger(Number(v));
    const esCodigoValido = (v: any) => /^[0-9]{3}$/.test(v);
    const esHoraValida = (v: any) => /^([01]\d|2[0-3]):[0-5]\d$/.test(v);
    const esLlenadoraValida = (v: any) => /^[0-9]{2}$/.test(v);
    const esCabezalValido = (v: any) => ["A", "B", "C"].includes(v);
