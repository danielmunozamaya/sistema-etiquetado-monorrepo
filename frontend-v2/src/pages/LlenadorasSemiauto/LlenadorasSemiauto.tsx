...
function LlenadorasSemiauto() {
  ...
  const datosRef = useRef<Llenadora[]>([]);

  const cargarAsociaciones = async () => {
    try {
      const asociaciones = await getAsociaciones();

      const llenadorasMap = new Map<string, Llenadora>();
      ...
      asociaciones.forEach((a: any) => {
        ...
        const cabezal: Cabezal = {
          ...
          comunicacion: a.cabezal?.comunicacion ?? null,
        };
      ...
      const elapsed = new Date().getTime() - startLoadingTime.current.getTime();
