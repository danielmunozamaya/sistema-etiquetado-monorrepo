...
const VerEans: React.FC<VerEansProps> = ({ userRole }) => {
  ...
  const [registros, setRegistros] = useState<VerEan[]>([]);
  const [filtrosParseados, setFiltrosParseados] = useState<Record<string, any>>(
    {}
  );
  const [eanToDelete, setEanToDelete] = useState<VerEan | null>(null);
  const [eanToDeleteNoItem, setEanToDeleteNoItem] = useState<number | null>(
    null
  );
  ...
  const validarDiasBestBefore = (desde: string, hasta: string) =>
  ...
  const setFetchBody = () => {
    const body: { where: Record<string, any> } = { where: {} };
  ...
  const fetchAndSetEans = async (body: Record<string, any>, newOffset = 0) => {
  ...
  const handleRowClick = (registro: VerEan, no_item: number) => {
